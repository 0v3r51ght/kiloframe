"""Optional cloud providers, used only when explicitly asked for.

KiloFrame's local route is Ollama. This module exists so a request can be escalated to a
larger hosted model on purpose -- when the local one is too slow or too small for the job
-- not so inference quietly moves off the machine.
...
* The answer says which model produced it, so an escalated reply is never mistaken for a
  local one.
...
class ProviderRegistry:
    ...
    def configure(self, name: str, api_key: str, model: str | None = None, account_id: str | None = None) -> Provider:
        ...
        if name == "cloudflare":
            account_id = (account_id or os.environ.get("KILOFRAME_CLOUDFLARE_ACCOUNT_ID", "")).strip()
