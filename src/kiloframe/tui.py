...
class TerminalUI:
    ...
    async def banner(self) -> None:
        ...
        try:
            ...
            model_name = str(status.get("model") or "no model") or "unknown"
        ...
        info = [
            ...
            f"{DIM}{KILOFRAME_CREDIT}{RESET}",
        ]
        width = self._width()
        art_width = visible_len(KILOFRAME_ART[0])
        ...
        if stacked:
            rows = [f"{GREEN}{BOLD}{art}{RESET}" for art in KILOFRAME_ART] + [m for m in info if m]
        else:
            padded = info + [""] * (len(KILOFRAME_ART) - len(info))
            rows = [f"{GREEN}{BOLD}{art}{RESET}   {self._fit(meta, width - 6 - art_width - 3)}" for art, meta in zip(KILOFRAME_ART, padded, strict=True)]
    ...
    async def _animate(self, state: dict[str, Any]) -> None:
        ...
        while True:
            ...
            model = state.get("model") or "Ollama"
            sys.stdout.write(
        ...
        while True:
            ...
            if text == "/help":
                self._panel("commands", PURPLE, [
                    ...
                    f"{GREEN}{'/exit':<8}{RESET} {DIM}leave KiloFrame{RESET}",
                    f"{DIM}anything else is sent to the local model · ctrl-c cancels{RESET}",
                ])
                continue
            if text == "/clear":
